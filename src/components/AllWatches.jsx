        ...
        <div>
            ...
            {watches.length > 0 && watches.map((watch, index) => {
                ...
                if (index === 0) {
                    ...
                    return (
                        'Sorry for the inconvienence, Image not avalible at this time'
                    )
                };

                return (
                    <div className="allWatches" key={watch.id} style={{ textAlign: 'center', border: 'solid black' , padding: '5px', width: '300px', height: '300px'}}>
                        <Link to={`/watches/${watch.id}`}>
                            <img src={imageloop} alt={`${watch.name}`} style={{ width: '160px', height: '160px', border: 'solid black 5px' }} />                           
                             <h2>{watch.brand}</h2>
                            <h2>{watch.name}</h2>
                            <h2>{watch.price}</h2>
                        </Link>
